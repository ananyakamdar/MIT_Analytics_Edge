...
```{r}
IBM = read.csv("MIT Analytics Edge/Unit 1/IBMStock.csv")
GE = read.csv("MIT Analytics Edge/Unit 1/GEStock.csv")
ProcterGamble = read.csv("MIT Analytics Edge/Unit 1/ProcterGambleStock.csv")
CocaCola = read.csv("MIT Analytics Edge/Unit 1/CocaColaStock.csv")
Boeing = read.csv("MIT Analytics Edge/Unit 1/BoeingStock.csv")
```
...
The first argument to the as.Date function is the variable we want to convert, and the second argument is the format of the Date variable. We can just overwrite the original Date variable values with the output of this function. Now, answer the following questions using the sd, str, and summary functions.
...
```{r}
plot(CocaCola$Date, CocaCola$StockPrice, type = "l", xlab = "Date",
     ylab = "Stock Price", main = "Stock price of CocaCola over time")
```
...
print(mean(IBM$StockPrice))
```
**Ans**: January, February, March, April and May
