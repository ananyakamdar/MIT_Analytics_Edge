...
testing = subset(climate_change, Year > 2006)
summary(testing$Year)
temp_reg = lm(Temp ~ MEI + CO2 + CH4 + N2O + CFC.11 + CFC.12 + TSI + Aerosols, data = training)
summary(temp_reg)
```
**Ans**: 0.7509
...
```{r}
temp_reg2 = lm(Temp ~ MEI + N2O + TSI + Aerosols, data = training)
summary(temp_reg2)
```
**Ans**: 0.02532
...
```{r}
best_temp_reg = step(temp_reg)
summary(best_temp_reg)
```
**Ans**: 0.7508
...
```{r}
preds = predict(best_temp_reg, newdata = testing)
SSE = sum((preds - testing$Temp)^2)
