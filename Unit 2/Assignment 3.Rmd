...
```{r}
FluTrain = read.csv("FluTrain.csv", stringsAsFactors = TRUE)
FluTrain[which(FluTrain$ILI == max(FluTrain$ILI)),1]
```
**Ans**: October 18, 2009

Which week corresponds to the highest percentage of ILI-related query fraction?
```{r}
FluTrain[which(FluTrain$Queries == max(FluTrain$Queries)),1]
```
**Ans**: October 18, 2009
...
```{r}
FluTest = read.csv("FluTest.csv", stringsAsFactors = TRUE)
PredTest1 = exp(predict(FluTrend1, newdata=FluTest))
...
```{r}
(FluTest[which(FluTest$Week == "2012-03-11 - 2012-03-17"),2] - 2.187378)/FluTest[which(FluTest$Week == "2012-03-11 - 2012-03-17"),2]
```
**Ans**: 0.04623814
...
summary(FluTrend2)
```
**Ans**: Intercept, queries, log(ILILag2). R^2 = 0.9063
...
```{r}
preds2 = exp(predict(FluTrend2, newdata = FluTest))
sqrt(mean((preds2-FluTest$ILI)^2))
```
**Ans**: 0.2942029
